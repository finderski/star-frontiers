...
- **Apply Damage is a two-phase workflow: pure preview → user confirmation → commit.** `previewDamageMitigation(rawAmount, types, actor)` performs NO writes; it walks active screens and armor and returns a structured `{ raw, final, damageTypes, layers, messages }` plan. `commitDamageMitigation(actor, layers)` performs the screen SEU consumption / armor accumulated-damage / deactivation / destruction writes from that plan. `applyDamageToActor` consumes the pre-built plan, applies the user-confirmed Final Damage to the actor pool, and handles zero-state status. The user confirmation dialog (`promptApplyDamageConfirmation`) runs on the CLICKING user's client in `requestApplyDamage` BEFORE GM/socket delegation — the GM never sees a player's confirmation dialog. Canceling writes nothing (no item updates, no actor update, no apply-flag mark). Do not move the dialog into the GM-side commit path, and do not call `commitDamageMitigation` inside `previewDamageMitigation`. The legacy `mitigateDamage(...)` export is a backward-compat shim only.
- **Robot health pool is Stamina, not Structure.** Per Alpha Dawn Expanded, robots have Stamina points (`system.abilities.sta`), not Structure points. `getActorHealthPool("robot")` returns `system.abilities.sta` with `living: false` (zero-state applies `sf-dead`). `buildHealthPoolUpdates` does NOT mirror damage into `system.structuralPoints` for robots. The legacy `structuralPoints` field remains on the robot schema pending a future cleanup migration but is no longer touched by the damage workflow. Do not reintroduce the structuralPoints mirror.
