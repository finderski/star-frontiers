...
- The attack dialog root is `<form class="star-frontiers attack-dialog">`. Theme selector blocks that style `.star-frontiers.sheet` / `.theme-retro` content must keep `.star-frontiers.attack-dialog` in scope as well so the dialog inherits the active sheet theme.
