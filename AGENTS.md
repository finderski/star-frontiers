...
- **Expanded melee knockout is a flagged hit outcome, not a live status application.** On an Expanded melee hit, `01`-`02` or any blunt/barehand hit ending in `0` (excluding `100`) stamps knockout metadata onto the shot model (`statusId: sf-unconscious`, `durationFormula: 1d100`). This phase only surfaces the flag on the chat card; later damage/status phases apply it.
