...
- The attack dialog must keep its current round-two layout: one `Modifiers` section with `Applies to All Shots` + `Per-Shot` subsections, plus the optional forced-d100 / GM-testing section. Do not reintroduce a separate `Manual Modifiers` card or the old `Adjust per-shot...` expander.
