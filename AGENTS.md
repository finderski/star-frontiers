...
- **Attack-card on-hit effects are now button-gated snapshots, not auto-fire side effects.** `rollWeaponAttack()` stores a frozen `onHitEffects[]` list plus `avoidance.resolved` / `avoidance.succeeded` on the attack-card model at roll time. The card only shows Apply Status buttons when the attack hit and either has no avoidance or a later avoidance roll updated the source attack card to `resolved && !succeeded`. Do not reintroduce auto-application on hit / avoidance failure.
