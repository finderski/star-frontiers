...
- Current schema version: **0.3.0** (stored in world setting `schemaVersion`).
...
## Current next tasks
- Creature sheet smoke-test (0.3.0):
  ...
  - Drag a `weapon` item onto a creature; confirm the Carried Weapons section appears, the row uses the normal weapon profile (skill/ability math), and the section disappears when the weapon is removed.
  ...
  - Confirm the 0.3.0 migration converts a pre-existing creature's inline `system.attacks[]` into embedded `creatureAttack` items and backfills `reactionSpeed`.
...
- `creatureAttack` items intentionally bypass the weapon-only fields. The shared `AttackPipeline` branches on `weapon.type === "creatureAttack"` in `getWeaponAttackProfile` (flat `system.attackScore` as `baseTarget`), `buildEffectiveDamageFormula` (reads `system.range.rangeBands[band].damageFormula` then `system.damageFormula`), `getAvailableWeaponRangeBands` / `getWeaponRangeBandFromDistance` (reads `system.range.rangeBands` only when `system.range.enabled`), `getAmmoConsumption` (returns 0), `getActiveWeaponMode` (returns null), `getWeaponOnHitEffectIds` / `getWeaponOnHitEffectOrigin` (reads `system.onHitEffectIds`), and `getWeaponAvoidance` (reads `system.avoidance`). Do not add weapon-style `ammo`, `mechanics.modes`, `weaponSkillKey`, `requiredSkillRef`, etc. to creatureAttack items; if a creature needs to consume SEU it should carry a real `weapon` item instead.
- The creature sheet is single-page (no tabs). Natural Weapons (creatureAttack items) have an Add button AND accept drops of reusable `creatureAttack` items from the items directory. Carried Weapons (`weapon` items) are drag-only and the entire section is hidden when the creature owns no weapons. Both render through the same row partial and delegate combat to the shared pipeline. Do not give Carried Weapons an Add button.
