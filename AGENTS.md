...
- **Avoidance checks**: `AttackPipeline.rollAvoidance` rolls against `target.system.abilities[ability].value` (current score, not base) and posts the result as the **target's** speaker, not the attacker's. Failed rolls attach `flags["star-frontiers"].avoidanceFailure` for future Active Effect application.
