...
- **Attack dialog graceful degradation.** There are no separate tactical/theater-of-mind modes. Range is derived from measured distance when both tokens are available, otherwise it falls back to a dialog dropdown. Target size is derived from the target actor when known, otherwise it falls back to a dialog dropdown. Cover and concealment must NEVER be inferred from scene geometry.
- **Attack dialog structure.** The weapon-attack popup has exactly TWO authoring sections: `Modifiers` and `Manual Modifiers`. Do not reintroduce a separate visible `Derived`/`Dialog Inputs` section just because modifier rows still carry `source` tags internally; the UI groups by purpose, not by storage source.
...
- **Player override setting.** `homebrewPlayerCanOverrideModifiers` controls whether non-GMs can toggle/edit derived modifier rows in the attack dialog. Dialog-native controls (movement, careful aim, GM Circumstance, Misc Modifier) stay editable regardless; the GM always has full access.
...
- **Wrong Hand special case.** `sf-wrong-hand` remains a registered status, but `appendStatusRows()` MUST skip emitting a status row for it. The dialog's `wrongHand` checkbox is the single per-attack source of truth; the status only seeds that checkbox's default checked state when the dialog opens.
- **Hard blockers.** Status-driven attack blockers (currently Stunned attacker-side, Unconscious) are pushed into `buildAttackModifierContext().blockers` rather than into the modifier list. The attack dialog renders a red banner listing each blocker and gates the Roll button until an override checkbox is set. GM Override is always available to GMs; Player Override is available to players only when `homebrewPlayerCanOverrideModifiers` is on (same setting that gates derived-modifier edits). Rolling under an override stamps `blockerOverride: { by: "gm" | "player", blockers: [...] }` on the chat card model so the table sees the rule was bypassed. Do not relax the player-side gate to "always available" — the homebrew setting is the deliberate switch.
...
- **Per-shot overrides.** Multi-shot attacks may optionally author `shots[i].modifierOverrides` inside `message.flags["star-frontiers"].attack`. That map is sparse and keyed by modifier id; an empty/missing map means the shot uses the attack-level modifier list unchanged. Compute shot-level TNs through `computeShotContext(...)` in `modifier-pipeline.mjs` instead of mutating the attack-level modifier array in place.
...
- Creature sheet smoke-test (0.3.1):
  ...
  - Verify on-hit AE application paths for creatureAttack (avoidance-enabled fail → AE applied, avoidance-disabled hit → AE applied).
...
- Ranged attack tightening smoke-test:
  ...
  - Verify Burst / Attacking From Behind / Target in Moving Vehicle / Telescopic Sight / Opportunity Shot only appear for the intended attack types and modify TN/chat-card notes correctly.
  - Verify multi-shot attacks can open `Adjust per-shot...`, override one shot's range or status value, and produce distinct per-shot target numbers plus chat-card override details.
- Attack roll rework:
...
- Weapons:
  ...
  - Foundry smoke-test on-hit / avoidance-failure Active Effect application end-to-end, especially GM-socket handoff when the attacker does not own the target
...
- The attack dialog must stay a two-section UI: `Modifiers` plus `Manual Modifiers`. Do not reintroduce a third visible section for `Derived`/`Dialog Inputs` just because the underlying modifier rows still track `source`.
- Wrong Hand is the ONLY status that does not emit its own status modifier row. Preserve the `sf-wrong-hand` status definition, but keep the runtime attack source of truth on the dialog checkbox so a single attack can opt in/out without mutating token state.
- Per-shot modifier overrides belong in `flags["star-frontiers"].attack.shots[i].modifierOverrides`; do not invent a parallel top-level shot-TN override model for the dialog. Shot-specific TNs should always be recomputed from the attack-level context plus that sparse override map.
