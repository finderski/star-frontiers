...
- Creature special attacks are descriptive notes in `system.specialAttacks[]` (`label` + `detail`), not separate `creatureAttack` items. Use `creatureAttack` items only for actual rolled natural attacks.
...
- Creature sheet smoke-test (0.3.1):
  ...
  - Specials: add/remove Special Attack entries, edit Special Defense multiselects/notes, and confirm the new prose fields persist.
...
- The creature sheet is single-page (no tabs). The header has a bare name field, then flex rows for Size+Type and Number Appearing+Native World+Habitat; there is no Identity fieldset. Natural Weapons (creatureAttack items) have an Add button AND accept drops of reusable `creatureAttack` items from the items directory. Carried Weapons (`weapon` items) are drag-only and the entire section is hidden when empty. Movement and Special Attacks are actor-array rows with explicit add/remove buttons, so future mutating handlers in those sections must keep arming `_rememberScrollPosition()` before updates.
