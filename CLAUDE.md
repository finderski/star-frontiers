...
- **Auto-miss:** roll 96–00 always misses.
...
| `automateActiveEffects` | bool | `true` | Enables Active Effect application flows such as racial-ability success toggles and chat-card Apply Status buttons for weapon on-hit effects |
| `autoHitUnconscious` | bool | `true` | In Alpha Dawn combat, attacks against an `sf-unconscious` target auto-hit on `01`-`95`; `96`-`00` still misses |
...
### Done
- **Apply Damage workflow polish.** Robot actors now take damage on `system.abilities.sta` rather than `system.structuralPoints` (Alpha Dawn Expanded: robots have Stamina points). `getActorHealthPool` returns `living: false` for robots; `buildHealthPoolUpdates` no longer mirrors damage into `structuralPoints`. The schema field stays in place pending a future cleanup migration. Mitigation was split: `previewDamageMitigation(rawAmount, types, actor)` is pure and returns a structured `{ raw, final, damageTypes, layers, messages }` plan; `commitDamageMitigation(actor, layers)` does the per-layer screen/armor item writes; `applyDamageToActor` consumes the plan (preferring a passed-in preview) and handles the actor pool update and zero-state status separately. A new client-side confirmation dialog (`promptApplyDamageConfirmation`) runs in `requestApplyDamage` BEFORE GM/socket delegation, showing target, raw damage, per-layer mitigation, an editable Final Damage field, and a live pool-before → pool-after preview. Cancel writes nothing. Confirm sends the captured `{ preview, finalDamage }` commit plan to the GM via the existing socket; `applyDamageFromMessage` commits the layer updates exactly as previewed and applies the user's override to the pool. The chat note now emits structured lines: Target, Original Damage, per-layer absorption (screen SEU consumption, armor accumulated/destroyed), Final Damage, Pool before → after, and any Dying/Dead status applied. The legacy `mitigateDamage(...)` export remains as a thin compatibility shim that previews then commits in one call.
- **Targetless attack follow-up buttons.** Avoidance roll, Apply Status (including avoidance-gated effects), and Apply Knock-Out buttons now render on the attack card whenever the attack hit and the weapon supports them, regardless of whether a target was selected at attack time. The chat-card handlers (`rollAvoidance`, `applyStatus`, `applyKnockout`) fall back at click time to `game.user.targets[0]` then `canvas.tokens.controlled[0]`; if neither is present they warn (`STARFRONTIERS.Weapon.AvoidanceNoTarget` / `STARFRONTIERS.Effects.NoTargetsToApply`) and bail. Avoidance-gated effects are no longer hidden from the attack card — both the attack card AND the avoidance failure card now offer the Apply Status button so users who roll avoidance manually from a sheet can still apply the status from the attack card. Per-message `applyKey` dedupe still prevents double-application from concurrent clicks.
- **Avoidance-gated status application was moved onto avoidance failure cards.** Attack cards now keep only non-avoidance on-hit Apply Status buttons and Apply Knock-Out independent of avoidance, while failed avoidance check cards render the deduped Apply Status button for the mapped Star Frontiers status (for example `sf-stunned`). `rollAvoidance()` now records that status mapping on the avoidance chat message so the shared `applyStatus` action works from either card type.
- **Powered melee SEU now spends on hit and reconciles through GM card edits.** The shared attack pipeline now treats `ammo.uses === "seu"` melee weapons as hit-based consumers: the pre-roll loaded-ammo check still reserves enough charge to attempt the swing, but the actual automated spend happens after the roll and uses `hitCount` instead of `shots`. Ranged/thrown weapons keep their existing per-use consumption path unchanged. Attack-card ammo state now records `perUnit`, `consumedUnits`, `spendOnHit`, `weaponUuid`, and `loadedSourceId`, and GM attack-card adjustments now reconcile that spend for powered melee only, including full refunds on hit→miss flips and re-charges on miss→hit flips, while clamping safely if the weapon/source changed in the meantime.
- **Damage/status application landed on combat chat cards.** Weapon damage now posts a dedicated damage card that carries its target context and exposes an Apply Damage button; attack cards now expose per-effect Apply Status buttons plus Apply Knock-Out when the stored melee KO flag is present. All three flows are GM-authoritative and deduped through `message.flags["star-frontiers"].applied`, so player clicks route through the system socket to the active GM, buttons disable after first use, and the same message cannot double-apply from concurrent GM/player clicks.
- **Expanded melee attack resolution.** Expanded melee now fully rides the shared attack dialog/chat-card pipeline instead of a future placeholder path. Non-creature melee keeps the shared DEX/STR selector, attacker Wounded now applies `-10` in Expanded melee as well as ranged, and `isHit()` now treats `01`-`05` as an automatic hit plus `96`-`00` as an automatic miss in both Basic and Expanded Alpha Dawn. Armed melee always resolves exactly one attack regardless of Rate of Fire, while barehand melee uses `actor.system.derived.barehandAttacks` as its attack cap and reuses the multi-shot UI with relabeled `Attacks` / `Per-Attack` / `Attack N` copy. Expanded melee attack results now also store `target.unconscious`, `autoHitUnconscious`, and per-attack knockout flags: `01`-`02` on a hit, or any blunt/barehand hit ending in `0` (excluding `100`), now surfaces `Unconscious for d100 turns` on the chat card and feeds the explicit Apply Knock-Out follow-up action.
- **Melee dialog modifier leak fix.** The shared dialog now explicitly excludes attacker movement, target movement, and `Firing Two Weapons` from melee instead of only hiding some controls. This removes the erroneous default `Stationary +10` from Expanded melee target numbers while keeping ranged behavior unchanged. Wrong Hand intentionally remains available for melee as the current house interpretation.
- **Knockout card surfacing + rolled duration.** Melee knockout results now surface in the always-visible region of the attack chat card next to the action buttons instead of inside `Show Details`. `createWeaponAttackChatMessage()` rolls a real `1d100` knockout duration exactly once when the card is first created, stores it on the attack model as `knockoutDuration`, and passes that roll through `chatData.rolls` so Dice So Nice can animate it. Later GM card adjustments re-render the same stored duration instead of re-rolling.
- **Knockout duration null-vs-zero fix.** The initial KO-duration guard and formatter now treat `null` / `undefined` / blank as “not rolled yet” instead of coercing them through `Number(...)`. This fixes the bug where a fresh knockout card showed `0 turns` and skipped the duration roll because `Number(null)` evaluated to `0`.
- **Weapon attack cards now expose real attack rolls to chat integrations.** The attack pipeline still renders custom roll HTML into the weapon attack card, but it now also passes the actual attack `Roll` objects into `chatData.rolls` when creating the message. This lets Dice So Nice animate the attack dice on the same custom card, alongside the already-rolled knockout duration die when present.
- **Race elasticity exposure + actor limbs groundwork.** Race item sheets now expose the `system.elasticity` block again (`available`, `limbsPerDexBucket`, `limbGrowMinutes`, `maxFiringLimbs`) while leaving `gliding` and `lightSensitivity` hidden. Character/NPC actors gained stored `system.limbsCurrent` plus derived `system.derived.isElastic`, `system.derived.maxLimbs`, and `system.derived.barehandAttacks`; the sheet only shows a Limbs input for elastic actors, and submit-time clamping keeps it within `4..maxLimbs` after DEX or race changes. Legacy `system.derived.derivedLimbs` is now just a compatibility mirror of `maxLimbs`. No schema migration was needed because the new stored field defaults cleanly and the rest is derived.
- **Weapon item sheet melee prep.** Weapon items now expose both `system.mechanics.barehand` and `system.mechanics.isBlunt` as melee-only checkboxes on the item sheet, and melee Type collapses the range editor to Point Blank only with read-only `0` / `2` reach values while leaving the hidden short/medium/long/extreme data intact for later switches back to ranged. The existing Modes UI was kept fully available for melee, and the UI copy was generalized from "Firing Modes" to "Modes" because melee and ranged now share the same per-mode editor. No schema migration was needed because these melee flags are optional defaulted fields and the Point Blank coercion happens at submit time.
...
- Phase 2 (core) — character sheet fully wired:
  ...
  - Auto-hit (01–05) and auto-miss (96–00) handled by `#isHit()` helper across Alpha Dawn attack flows
  ...
  - **Universal item Active Effects editor.** The generic embedded Active Effects editor now renders once near the bottom of every item sheet instead of living inside the `trainedAbility` section. Its shared Add/Open/Delete actions still operate on item-owned embedded AEs; delete also clears item-local refs from weapon on-hit lists, `creatureAttack.system.onHitEffectIds`, and consumable `system.effectIds`. Weapon mode and `creatureAttack` on-hit effect editors remain separate because they link specific effect IDs into mode/attack runtime behavior.
  ...
  - **trainedAbility item sheet additions** — added `rollType` dropdown (Active / Passive) in the four-column grid row alongside `baseChance`, `cap`, `xpPerPoint`. Racial Ability sheets now use the same universal item Active Effects editor as every other item type.
...
- On-hit AE runtime rule: active-mode `onHitEffectIds` take precedence over top-level `mechanics.onHitEffectIds`, but they are no longer auto-applied. The attack card snapshots the effect refs/names at roll time and surfaces explicit Apply Status buttons after a hit for every on-hit effect (including avoidance-gated ones). Avoidance failure chat cards also render the mapped status button so the same status is applyable from either card. Applied target effects are deduped/refreshed by `flags["star-frontiers"].appliedFrom`, and the per-message `applyKey` keeps the same chat-card button from double-applying.
...
### Not yet started
- Phase 3 remaining combat work — deeper medical/status/healing follow-through and creature/NPC sheet integration with the extracted attack pipeline
...
- **Weapon modes editor runtime smoke test.** `npm run check` is green, but the new Weapon item-sheet Modes editor still needs a live Foundry pass: add/remove modes, rename the active mode key, toggle `Has Modes` off/on without data loss, verify embedded Active Effect create/open/remove persistence, confirm authored rounds-per-shot values consume correctly in play, and verify melee Type shows the Barehand checkbox plus Point Blank-only reach without disturbing the shared modes editor.
- **Damage/status application runtime smoke test.** `npm run check` is green, but the new Apply Damage / Apply Status / Apply Knock-Out path still needs a live Foundry pass: confirm targeted damage, no-target GM multi-select application, player-owned target self-apply through the active GM socket, failed-vs-successful avoidance gating on the attack card, dedup when GM and player click together, and armor/screen mitigation edge cases (screen depletion, armor destruction, living `sf-dying`, non-living `sf-dead`).
...
- **Consumable effect-use mapping** — all item sheets now expose embedded Active Effect authoring, including consumables. The character-sheet Use flow still applies refs from `system.effectIds`, so a future pass should decide whether consumables need a dedicated picker that maps embedded effects into that use-time list.
