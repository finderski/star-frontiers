...
### Done
- **Weapon item sheet melee prep.** Weapon items now expose `system.mechanics.barehand` as a melee-only checkbox on the item sheet, and melee Type now collapses the range editor to Point Blank only with read-only `0` / `2` reach values while leaving the hidden short/medium/long/extreme data intact for later switches back to ranged. The existing Modes UI was kept fully available for melee, and the UI copy was generalized from "Firing Modes" to "Modes" because melee and ranged now share the same per-mode editor. No schema migration was needed because `mechanics.barehand` is an optional defaulted field and the Point Blank coercion happens at submit time.
