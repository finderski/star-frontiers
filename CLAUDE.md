...
- Phase 2 (core) — character sheet fully wired:
  ...
  - **0.2.4 — `currentChance` moved from `trainedAbility` item schema to character actor.** The field was character-progression state (actor-owned) living on an item template (wrong layer). Removed from `StarFrontiersTrainedAbilityData`; added `system.racialSkillProgress` (`ObjectField`) to `StarFrontiersCharacterData` — an arbitrary map keyed by trainedAbility item ID, each entry holding `{ currentChance: number }`. Migration 0.2.4 reads `_source.system.currentChance` from existing actor-owned `trainedAbility` items and writes into the actor map before the field is gone. Also added `objectField()` helper to `module/data/fields.mjs`. Item sheet `trainedAbility` section now shows `baseChance`, `cap`, `xpPerPoint` only (3 columns, not 4).
  - **0.2.4 — migrations.mjs syntax fix** — a stray `  }` was left after the 0.2.4 migration's closing brace (from the previous session's edit), producing a JS SyntaxError that prevented the entire system from loading. Removed the extra brace; the migration array is now valid.
  - **0.2.5 — Skill category choices converted** — `StarFrontiersSkillData.category` choices changed from `["racial", "psa", "general"]` to `["main", "subskill"]`; initial value is `"main"`. Migration 0.2.5 walks world items, actor items, unlinked-token raw source, and `invalidDocumentIds`, mapping any of the three old values → `"main"`.
  ...
  - **Skill sheet redesign** — reduced to a single 3-column row (PSA | Category | Roll Formula). Removed: Level, Ability, Bonus, Weapon Skill dropdown, Heavy Skill checkbox. Added: a sub-skill drop zone (visible only when `category === "main"`) backed by `system.subskillRefs` (array of IDs/UUIDs to other skill items with `category === "subskill"`). The `weaponSkillKey` field remains in the schema (hidden from sheets) for backward compat with the existing attack roll code.
  ...
  - **Racial Abilities section on character sheet (Expanded only)** — the Skills+Equipment tab now shows a Racial Abilities fieldset when the character owns any `trainedAbility` items. Each row shows: item name, current chance (from `racialSkillProgress[id]?.currentChance ?? item.system.baseChance`), a roll-die button (for `rollType: "active"` items), and a fire button when the triggering AE is active (clicking it re-disables the AE). Rolling 1d100 ≤ chance posts a check-roll chat card and, on success, sets the item's triggering AE to `disabled: false`.

### Skill data model (current)
- `category` choices: `main` · `subskill` (default `"main"`). Old values `racial/psa/general` were all mapped to `"main"` in migration 0.2.5.
...
- `rollFormula`: free text; shown on item sheet for all categories.
...
- **Racial Ability model shape** — the UI label is now `Racial Ability`, but the underlying item type is still `trainedAbility`. That is intentional for now. Remaining design question: long-term fate of `cap`, `xpPerPoint` — keep on sheet, or partly move to Active Effects semantics?
- **Battle Rage / racial ability rolls — partially done.** Roll UI is implemented (character sheet Racial Abilities section, 1d100 vs chance, chat card, AE enable on success, End Rage button to re-disable). Still missing: wire `combatProfile.meleeBonus` into the attack roll so the AE's +20 actually modifies the to-hit; XP-based `currentChance` improvement workflow; "End Rage" only shows when AE is active — needs testing in Foundry to confirm the AE transfer/disabled flag cycle works as expected.
