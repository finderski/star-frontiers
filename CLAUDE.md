...
- Phase 2 (core) — character sheet fully wired:
  ...
  - **0.2.6 — Racial Ability storage cleanup at base.** Fixed `#performRacialAbilityChanceAdjustment` so returning an ability's chance to base actually removes its `racialSkillProgress` entry. Previous `deepClone` + JS `delete` pattern was silently undone by Foundry's deep-merge; replaced with per-key update paths using `-=` deletion syntax (`system.racialSkillProgress.-=<itemId>`).
