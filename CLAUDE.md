...
module/migration/migrations.mjs Schema migration runner — current version 0.3.1
...
- Phase 2 (core) — character sheet fully wired:
  ...
  - **0.2.8 — Token double-right-click targeting shortcut.** Double-right-clicking a token now targets it directly from the canvas. Holding `Shift` preserves existing targets so the shortcut still supports multi-target selection instead of always replacing the target set.
