...
- **Racial Ability model shape** — the UI label is now `Racial Ability`, but the underlying item type is still `trainedAbility`. That is intentional for now. Remaining design question: which fields should stay on the sheet long-term (`baseChance`, `currentChance`, `cap`, `xpPerPoint`) versus being moved to actor-owned/runtime data or replaced with Active Effects semantics.
