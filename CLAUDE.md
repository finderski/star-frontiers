...
### Done
- **Ranged attack pipeline tighten-up.** Consolidated the attack popup into two sections (`Modifiers` + `Manual Modifiers`) while keeping the internal `derived | status | dialog | manual` source tags for recompute/chat-card use. Wrong Hand is now a single dialog checkbox auto-seeded from `sf-wrong-hand` instead of a separate status row, and the missing situational choices were added to the dialog: Burst Fire (+20 when the active weapon supports burst), Attacking From Behind (+20 melee), Target in Moving Vehicle (-10 ranged), Telescopic Sight (range-band downgrade with note), and Opportunity Shot (cancels target movement penalty but still leaves a `0`-value note row for auditability). Multi-shot attacks also gained an `Adjust per-shot...` expander that stores sparse `shots[i].modifierOverrides` maps on the structured attack chat-card model and recomputes each shot's TN independently through `computeShotContext(...)`. Added a future-work entry for world-level range-band overrides to complement the existing target-size settings.
